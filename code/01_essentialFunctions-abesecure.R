...
processFastaSample_ABE <- function(simple_name, step1_br_filter_file, dna_variants, gtf_forward, gtf_reverse, pad = 50){
  ...
  ss_g <- 1:length(gr)
  possible_forward <- as.vector(ss_g %in% queryHits(pos_ov)  & ss_g %ni% queryHits(neg_ov) & ref == "A"  & seqnames(gr) %in% canonical)
  possible_reverse <- as.vector(ss_g %in% queryHits(neg_ov)  & ss_g %ni% queryHits(pos_ov) & ref == "T"  & seqnames(gr) %in% canonical)
  ...
  df_forward$canonical_bases <- df_forward$`G` +  df_forward$A
  df_forward$editRate <- df_forward$`G`/( df_forward$TOTAL)
  ...
  df_reverse$canonical_bases <- df_reverse$C +  df_reverse$`T`
  df_reverse$editRate <- df_reverse$C/( df_reverse$TOTAL)
  ...
  lout <- lapply(possible_chrs, function(chr){
    ...
    # Write to fasta
    fasta_file <- paste0("../fastas/ABE-sequence/", simple_name, "." ,chr, ".fasta")
  ...
  write.table(qc_df, file = paste0("../fastas/ABE-qc/", simple_name, "-qc.tsv"),
